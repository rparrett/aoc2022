...
impl Shape {
    ...
    pub fn get_choice_for_outcome(&self, outcome: &Outcome) -> Shape {
...
impl Outcome {
    fn from_strategy_guide(outcome: &str) -> anyhow::Result<Self> {
        match outcome {
            "X" => Ok(Self(Ordering::Less)),
            "Y" => Ok(Self(Ordering::Equal)),
            "Z" => Ok(Self(Ordering::Greater)),
            _ => Err(anyhow!("invalid outcome: {}", outcome)),
        }
    }
}
...
fn main() -> anyhow::Result<()> {
    ...
    for line in BufReader::new(file).lines().flatten() {
        let (opponent, outcome_str) = line
            .split_once(' ')
            .ok_or_else(|| anyhow!("parsing failed"))?;

        let opponent_choice = Shape::from_opponent_choice(opponent)?;
        let outcome = Outcome::from_strategy_guide(outcome_str)?;

        let your_choice = opponent_choice.get_choice_for_outcome(&outcome);
